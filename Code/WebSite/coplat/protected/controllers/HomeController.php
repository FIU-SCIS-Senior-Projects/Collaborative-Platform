...
class HomeController extends Controller
{
    ...
    public function actionuserHome()
    {
        ...
        $TicketsO = Ticket::model()->findAllBySql("SELECT * FROM ticket left join (select ticket_id, event_recorded_date from (select * from ticket_events where event_performed_by_user_id !=:id and (event_type_id != 8 and event_type_id !=9) order by event_recorded_date desc)x group by ticket_id)x on ticket.id = ticket_id WHERE (assign_user_id=:id or creator_user_id=:id)
        and (status=:pending or status=:reject) order by event_recorded_date desc", array(":id" => $user->id,":pending"=>'pending', ":reject"=>'reject' ));
